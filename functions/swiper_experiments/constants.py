...
class Text:
    ...
    TALK_NOT_FOUND = f"<i>💔 Розмову не знайдено</i> /{Commands.HELP}"
    TALK_STOPPED = "🚧 Ця функція поки що не реалізована"

    NEW_TOPIC_STARTED = (
        f"<i>Ви створили нову тему для розмов - очікуйте відповідей ⏳</i> /{Commands.HELP}"
    )
    REVOKE_TOPIC = "⛔️Відкликати"

    MESSAGE_NOT_TRANSMITTED = f"<i>Повідомлення не відправлено 😞</i> /{Commands.HELP}"
    FAILED_TO_EDIT_AT_RECEIVER = f"<i>Не вдалося відредакгувати у отримувача 😞</i> /{Commands.HELP}"
